...
class ModelBuilderTest extends Properties("Flatten_GDL_functional") {
  import ModelBuilder.flatten
  ...
  property("retain_inits") = forAll(Gen.listOf(inits(termsContaining(Set())))) { inits =>
    ...
    flatten(description).rules.size == inits.size
  }
  ...
  property("substitute_multiple_inits") = forAll(Gen.nonEmptyListOf(inits(constants)), variables) { (inits, variable) =>
    ...
    flatten(description).rules.size == inits.size + inits.toSet.size
  }
  ...
  property("introduce_object_constants") = forAll(Gen.nonEmptyListOf(inits(constants)), constants, variables) { (inits, constant, variable) =>
    ...
    !inits.flatMap(_.head.terms).contains(constant) ==> (flatten(description).rules.size == inits.size + inits.toSet.size + 1)
  }
  ...
  property("multiply_object_constants") = forAll(nonEmptyListOfMax(10, inits(constants)), functions(variables)) { (inits, function) =>
    ...
    flatten(description).rules.size == inits.size + Math.pow(inits.toSet.size, variables.size)
  }
  ...
  property("ignore_distinct_with_equal_constants") = forAll(atomicSentences(constants), termsContaining(Set())) { (head, term) =>
    val description = Description(Seq(Rule(head, Seq(Distinct(term, term)))))

    flatten(description).rules.size == 0
  }

  property("filter_object_constants") = forAll(Gen.nonEmptyListOf(inits(constants)), constants, variables) { (inits, constant, variable) =>
    val description = Description(inits ++ Seq(
      Rule(Next(constant), Seq(True(variable), Distinct(constant, variable)))
    ))

    !inits.flatMap(_.head.terms).contains(constant) ==> (flatten(description).rules.size == inits.size + inits.toSet.size)
  }
